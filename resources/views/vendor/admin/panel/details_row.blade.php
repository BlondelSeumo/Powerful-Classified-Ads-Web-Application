        ...
        <div class="col-md-11">
                    ...
                    @foreach($translations as $key => $entry)
                        ...
                        <tr>
                            ...
                            @foreach($xPanel->columns as $column)
                                ...
                                @elseif (isset($column['type'], $column['function_name']) && $column['type']=='model_function')
                                    <td>
                                        <?php
                                            $columnValue = $entry->{$column['function_name']}();
                                            $columnValue = trim(strip_tags($columnValue, '<i>'));
                                            if (!\Illuminate\Support\Str::endsWith($columnValue, '</i>')) {
                                                $columnValue = \Illuminate\Support\Str::limit($columnValue, 80, "[...]");
                                            }
                                            echo $columnValue;
                                        ?>
                                    </td>
                            ...
                            @if ($xPanel->hasAccess('update') or $xPanel->hasAccess('delete'))
                                <td>
                                    @if ($xPanel->hasAccess('update'))
                                        <a href="{{ str_replace('/'.$original_entry->id, '/'.$entry->id, str_replace('details', 'edit', request()->url())) }}"
            ...
            @if ($languages_to_translate_in->count())
                ...
                @foreach($languages_to_translate_in as $lang)
                    <a href="{{ str_replace('details', 'translate/'.$lang->abbr, request()->url()) }}" class="btn btn-xs btn-secondary">
